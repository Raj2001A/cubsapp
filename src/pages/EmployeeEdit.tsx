...
const EmployeeEdit: React.FC = () => {
  ...
  useEffect(() => {
    if (!id) return;
    setLoading(true);
    apiService.get<Employee>(`/employees/${id}`)
      .then(res => {
  ...
  const validate = async () => {
    ...
    // Uniqueness check for employeeId (ignore current employee)
    if (employee?.employeeId) {
      try {
        const res = await apiService.get(`/employees?employeeId=${encodeURIComponent(employee.employeeId)}`);
        if (res.data && Array.isArray(res.data)) {
          const found = res.data.find((e: any) => e.employeeId === employee.employeeId && e.id !== id);
          if (found) newErrors.employeeId = 'Employee ID must be unique';
        }
      } catch {}
    }
    return newErrors;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const response = await apiService.put<Employee>(`/employees/${id}`, employee);
      if (response.data) {
  ...
  if (!employee) return <div className="flex justify-center items-center h-64">Employee not found</div>;
  ...
  return (
    <div className="p-6 min-h-screen flex items-center justify-center bg-gray-50">
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-8 w-full max-w-xl space-y-6">
        <h2 className="text-xl font-bold mb-4" tabIndex={0} aria-label="Edit Employee">Edit Employee</h2>
        ...
        <div className="flex justify-end gap-2 mt-4">
          <button type="button" onClick={() => navigate('/employees')} className="btn btn-secondary focus:ring-2 focus:ring-offset-2 focus:ring-blue-500" aria-label="Cancel edit">Cancel</button>
          <button type="submit" className="btn btn-primary focus:ring-2 focus:ring-offset-2 focus:ring-blue-500" disabled={saving} aria-label="Save changes">{saving ? 'Saving...' : 'Save Changes'}</button>
        </div>
      </form>
    </div>
  );
